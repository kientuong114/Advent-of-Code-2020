...
dirs = [coord for coord in itertools.product(range(-1, 2), range(-1, 2), range(-1, 2)) if coord != (0,0,0)]

def nbs(mtx, loc):
    return sum(1 for d in dirs if (loc[0] + d[0], loc[1] + d[1], loc[2] + d[2]) in mtx)
